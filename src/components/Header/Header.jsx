...
function getShouldRenderInteractiveScene() {
...
function HeroSceneFallback() {
  return (
    ...
    >
      <span
        className={`${styles.fallbackBadge} ${styles.fallbackBadgeReact}`}
      >
        React
      </span>
      <span
        className={`${styles.fallbackBadge} ${styles.fallbackBadgeNext}`}
      >
        Next.js
      </span>
      <span
        className={`${styles.fallbackBadge} ${styles.fallbackBadgeTailwind}`}
      >
        Tailwind
      </span>
    </div>
...
function Header({ refProps, projectsRef, projectsSectionId, handleNavigate }) {
  const [scenePointer, setScenePointer] = useState(neutralPointer);
  const [shouldRenderInteractiveScene, setShouldRenderInteractiveScene] = useState(
    getShouldRenderInteractiveScene,
  );
  ...
  useEffect(() => {
    ...
    const updateSceneAvailability = () => {
      setShouldRenderInteractiveScene(getShouldRenderInteractiveScene());
    };

    updateSceneAvailability();
    window.addEventListener("resize", updateSceneAvailability);

    if (reducedMotionQuery.addEventListener) {
      reducedMotionQuery.addEventListener("change", updateSceneAvailability);
      finePointerQuery.addEventListener("change", updateSceneAvailability);
    } else {
      reducedMotionQuery.addListener(updateSceneAvailability);
      finePointerQuery.addListener(updateSceneAvailability);
    }

    return () => {
      window.removeEventListener("resize", updateSceneAvailability);

      if (reducedMotionQuery.removeEventListener) {
        reducedMotionQuery.removeEventListener("change", updateSceneAvailability);
        finePointerQuery.removeEventListener("change", updateSceneAvailability);
      } else {
        reducedMotionQuery.removeListener(updateSceneAvailability);
        finePointerQuery.removeListener(updateSceneAvailability);
      }
    };
  }, []);
  ...
  return (
    ...
    >
      ...
      <article className={styles.introductionContainer}>
        ...
        <ShadowButton
          handleClick={() => handleNavigate(projectsRef, projectsSectionId)}
        >
          See my work
        </ShadowButton>
      </article>
